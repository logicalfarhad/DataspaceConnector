    ...
    private class DataAccessor implements IAccessor<Integer>{
        ...
        private Map<String, DataType<?>> getTypes() {
            ...
            for (int i=0; i<handle.getNumColumns(); i++){
                ...
                io.dataspaceconnector.arx.DataType<?> type = definition.getDataType(attribute);
                if (type instanceof io.dataspaceconnector.arx.DataType.ARXDecimal){
                    String format = ((ARXDecimal)type).getFormat();
                    result.put(attribute, DataType.NUMERIC(format));
                } else if (type instanceof io.dataspaceconnector.arx.DataType.ARXInteger) {
                    result.put(attribute, DataType.NUMERIC);
                } else if (type instanceof io.dataspaceconnector.arx.DataType.ARXString) {
                    result.put(attribute, DataType.STRING);                    
                } else if (type instanceof io.dataspaceconnector.arx.DataType.ARXDate){
